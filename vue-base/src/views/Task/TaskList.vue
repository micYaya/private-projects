  ...
  <div class="list-container">
    ...
    <div class="lower-section">
      ...
      >
        ...
        <el-table-column prop="item_count" label="实验项目数量" width="200" />
        <el-table-column label="操作" fixed="right" min-width="400">
          <template #default="scope">
            <el-button
              type="primary"
              @click="
                goToItemList(scope.row.id, scope.row.deviceId, scope.row.status)
              "
            >
              实验项目管理
            </el-button>
            ...
            <el-button type="danger" @click="deleteConfirm(scope.row)">
              删除任务
            </el-button>
          </template>
        </el-table-column>
...
const handleSelectionChange = (rows) => {
  ...
  isIndeterminate.value =
    rows.length > 0 && rows.length < taskList.value.length;
